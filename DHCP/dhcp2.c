...
void sendOffer(int acao, char *mac){
	...
	dhcpmsg.ciaddr = 0;
	dhcpmsg.yiaddr = inet_addr("192.168.1.28");
	dhcpmsg.siaddr = 0;
	dhcpmsg.giaddr = 0;
	...
	dhcpmsg.opt[33]=15;
	dhcpmsg.opt[34]=11;
	dhcpmsg.opt[35]=100;//64
	dhcpmsg.opt[36]=111;//6f
	dhcpmsg.opt[37]=109;//6d
	dhcpmsg.opt[38]=97;//61
	dhcpmsg.opt[39]=105;//69
	dhcpmsg.opt[40]=110;//6e
	dhcpmsg.opt[41]=46;//2e
	dhcpmsg.opt[42]=110;//6e
	dhcpmsg.opt[43]=97;//61
	dhcpmsg.opt[44]=109;//6d
	dhcpmsg.opt[45]=101;//65
...
void subString (char* input, int offset, int len, char* dest)
{
...
void sendMisto(int acao, char *mac,char pos4){
	...
	subString(&mac[4],6,2,&pos4);

	printf("MAC Address: %x:%x:%x:%x:%x:%x \n",mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	printf("MAC Address:sadasdas %x \n",pos4);
	/* Configura MAC Origem e Destino */
	MacAddress localMac = {0xDC, 0x53, 0x60, 0x13, 0x99, 0x8A};
	//MacAddress destMac = {mac[0], mac[1], mac[2], mac[3], pos4, mac[5]};
	...
	struct iphdr *ip;// = (struct iphdr *) (etherTypeT + sizeof(struct ether_header));
    ...
    ip->tot_len = htons(sizeof(struct iphdr) + sizeof(struct dhcpmessage));// + sizeof(struct udphdr)  + sizeof(struct dhcp_packet));
	...
	memcpy((buffer+sizeof(etherTypeT)), &(ip), sizeof(ip));

	/* Add some data */
	memcpy((buffer+ETHERTYPE_LEN+(2*MAC_ADDR_LEN)), &dummyBuf, sizeof(dummyBuf));

	//while(1) {
		/* Envia pacotes de 64 bytes */
		if((retValue = sendto(sockFd, buffer, sizeof(buffer), 0, (struct sockaddr *)&(destAddr), sizeof(struct sockaddr_ll))) < 0) {
			printf("ERROR! sendto() \n");
			exit(1);
		}
			...
			if(buff1[23] == 0x11){ //UDP
				...
				if(buff1[next_p+3] == 0x43){
					if(buff1[next_p+250] == 0x01){
						...
						sendMisto(0,mac,buff1[next_p+258]);
						...
						if(buff1[next_p+250] == 0x03){
							//sendOffer(1,mac);
							sendMisto(1,mac,buff1[next_p+258]);
							printf("MAC Address: %x:%x:%x:%x:%x:%x \n",buff1[next_p+254],buff1[next_p+255],buff1[next_p+256],buff1[next_p+257],buff1[next_p+258],buff1[next_p+259]);
							printf("DHCP Request  \n"/*,buff1[next_p+3],buff1[next_p+250]*/);
						}
					}
